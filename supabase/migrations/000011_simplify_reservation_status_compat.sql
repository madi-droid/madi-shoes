-- Удаляем неиспользуемый параметр из совместимого обновления статуса заявки.
drop function if exists public.update_reservation_status_compat(text, text, text, text, integer, text, text, timestamptz, text);
...
revoke all on function public.update_reservation_status_compat(text, text, text, integer, text, text, timestamptz, text) from public;
grant execute on function public.update_reservation_status_compat(text, text, text, integer, text, text, timestamptz, text) to anon, authenticated;
